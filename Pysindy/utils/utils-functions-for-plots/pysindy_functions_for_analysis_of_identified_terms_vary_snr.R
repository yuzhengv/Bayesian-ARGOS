...
# ~ ----------------------------------------------------------------------------
# - Functions used to select results of specific methods and a typical range of snr values
selected_variables_summary_info <- function(
    ...
    selected_results_summary_df <- total_results_summary_df %>%
        ...
        dplyr::filter(snr %in% selected_snr_list)
...
selected_variables_info <- function(
    ...
    selected_variables_info <- selected_variables_summary_info(
        total_results_summary_df = total_results_summary_df,
        method_name = method_name,
        selected_snr_list = selected_snr_list
    )
...
plot_freq_of_identified_terms_circular_layered <- function(
    ...
    circos.track(
        ...
        panel.fun = function(x, y) {
            ...
            # Handle infinite SNR (no noise) case
            if (snr_val == "Inf") {
    ...
    # mtext(
    #     paste(
    #         "Layout: SNR values as sectors, Equations as concentric layers (Eq1 outermost)"
    #     ),
    #     side = 1,
    #     line = 2,
    #     cex = 0.7,
...
plot_freq_of_identified_terms_circular_equations_as_sectors <- function(
    ...
    use_sector_colors = FALSE # New parameter: use sector colors instead of snr colors for bars
) {
    true_terms_list <- list(
        true_terms_1,
        true_terms_2,
    ...
    if (snr_order == "ascending") {
        snr_label_colors <- colorRampPalette(c(
            "#ee7959",
            "#b93a26",
            "#9e2a22"
        ))(
                    ...
                    panel.fun = function(x, y) {
                        ...
                        if (CELL_META$sector.numeric.index == 1) {
                            if (snr_val == Inf) {
                                ...
                                snr_label <- as.character(snr_val)
                            }
                            circos.text(
    ...
    if (use_sector_colors) {
        ...
        legend_labels <- c(
            "Ground Truth",
            paste0("SNR = ", ifelse(snr_values == Inf, "∞", snr_values))
        )
                ...
                circos.track(
                    ...
                    panel.fun = function(x, y) {
                        ...
                        if (nrow(eq_snr_data) > 0) {
                            ...
                            # Simplified positioning: create linear alignment from inner to outer
                            # Calculate bar width based on maximum possible terms across all equations
                            if (snr_idx >= 5 && num_terms > 9) {
                                bar_width <- 3.7 / max_num_terms
                            } else if (snr_idx >= 4 && num_terms > 8) {
                                bar_width <- 2.7 / max_num_terms
                            } else if (snr_idx >= 3 && num_terms > 7) {
                                bar_width <- 1.5 / max_num_terms
                            } else {
                                bar_width <- min(0.8 / max_num_terms, 0.09)
                            }

                            # Create linear positions from left (inner) to right (outer)
                            if (num_terms == 1) {
                                # Single term - position it towards the inner side
                                x_positions <- 0.025 + bar_width / 2
                            } else {
                                # Multiple terms - create linear progression from inner to outer
                                start_pos <- 0.025 + bar_width / 2
                                end_pos <- min(
                                    0.975 - bar_width / 2,
                                    start_pos +
                                        (num_terms - 1) * bar_width * 1.2
                                )
                                # end_pos <- start_pos +
                                #     (num_terms - 1) * bar_width * 1.1 * 1.2
                                x_positions <- seq(
                                    start_pos,
                                    end_pos,
                                    length.out = num_terms
                            ...
                            base_cex_multiplier <- min(
                                2.5,
                                current_layer_height / base_cex_index
                            ) # Increased base multiplier
                            ...
                            for (term_idx in seq_len(num_terms)) {
                                ...
                                if (
                                    ...
                                    # Calculate the y-position for term labels
                                    label_y <- max(
                                        term_data$Freq + track_y_max * 0.005,
                                        track_y_max * 0.870
                                    )
                        ...
                        if (CELL_META$sector.numeric.index == 1) {
                            ...
                            if (snr_idx == 1) {
                                ...
                                circos.text(
                                    ...
                                    y = track_y_max + 80,
                                    labels = vary_factor,
                                    cex = 1.625 * base_cex_multiplier, # Increased from 0.6
                                    col = snr_colors[snr_idx],
                                    font = 2,
                                    adj = c(0.60, 0),
                                    facing = "inside",
                    ...
                    panel.fun = function(x, y) {
                        ...
                        if (nrow(eq_snr_data) > 0) {
                            ...
                            if (use_special_positioning == FALSE) {
                                ...
                                if (snr_idx >= 5 && num_terms > 4) {
                                    bar_width <- 0.65 / max_num_terms
                                } else if (snr_idx >= 4 && num_terms > 8) {
                            ...
                            if (use_special_positioning == FALSE) {
                                adaptive_cex_terms <- if (num_terms <= 2) {
                                    ...
                                    0.55 * snr_shrinkage_factor # Increased from 0.35
                                }
                            } else {
                                adaptive_cex_terms <- if (num_terms <= 2) {
                                    0.6 * snr_shrinkage_factor # Increased from 0.7
                                } else if (num_terms <= 4) {
                                    0.5 * snr_shrinkage_factor # Increased from 0.6
                                } else if (num_terms <= 6) {
                                    0.5 * snr_shrinkage_factor # Increased from 0.5
                                } else if (num_terms <= 8) {
                                    0.45 * snr_shrinkage_factor # Increased from 0.4
                                } else {
                                    0.35 * snr_shrinkage_factor # Increased from 0.35
                                }
                            }
