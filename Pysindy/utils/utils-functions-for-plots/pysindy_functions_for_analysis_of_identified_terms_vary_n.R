...
selected_variables_info <- function(
    ...
    total_results_summary_df <- enhance_total_results_summary_df_py(
        root_path = root_path,
        experiment_name_list = experiment_name_list,
        method_list = method_list,
        num_init = num_init,
    ...
    selected_variables_info <- selected_variables_summary_info(
        total_results_summary_df = total_results_summary_df,
        method_name = method_name,
        selected_eta_list = selected_eta_list
    )
...
count_terms_identified_of_single_function_at_specific_eta <- function(
    total_results_summary_df,
    method_name_list,
    selected_eta_list,
    function_id_list
...
convert_term_to_latex_orig <- function(term) {
    # Remove leading/trailing whitespace
    term <- trimws(term)

    # Handle function names with parentheses like "sin(1 x1)" -> "$\sin(x_1)$"
    term <- gsub(
        "^([a-zA-Z]+)\\(1 ([a-zA-Z])([0-9]+)\\)$",
        "$\\1(\\2_{\\3})$",
        term
    ...
    # Handle function names with parentheses like "sin(x1)" -> "$\sin(x_1)$"
    term <- gsub(
        "^([a-zA-Z]+)\\(([a-zA-Z])([0-9]+)\\)$",
        "$\\1(\\2_{\\3})$",
        term
    ...
    # Handle space-separated products with powers like "x0 x1 x2^2" -> "$x_0x_1x_2^2$"
    # First, handle individual variables with powers in space-separated context
    term <- gsub("([a-zA-Z])([0-9]+)\\^([0-9]+)", "\\1_{\\2}^{\\3}", term)

    # Then handle individual variables without powers in space-separated context
    term <- gsub("([a-zA-Z])([0-9]+)(?!_|\\^)", "\\1_{\\2}", term, perl = TRUE)

    # Remove spaces between variables to create products
    term <- gsub("\\s+", "", term)

    # Handle old format patterns for backward compatibility
    ...
    # If we have underscores or functions, wrap in LaTeX
    if ((grepl("_", term) || grepl("\\\\", term)) && !grepl("^\\$", term)) {
        term <- paste0("$", term, "$")
    }
...
convert_term_to_latex_complicated <- function(term) {
    # Remove leading/trailing whitespace
    term <- trimws(term)
    ...
    # Handle constant term "1" -> "$C$"
    if (term == "1") {
        return("$C$")
    }

    # Handle function names with parentheses like "sin(1 x1)" -> "$sin(x_1)$"
    if (grepl("^[a-zA-Z]+\\(1 [a-zA-Z][0-9]+\\)$", term)) {
        matches <- regmatches(
            term,
            regexec("^([a-zA-Z]+)\\(1 ([a-zA-Z])([0-9]+)\\)$", term)
        )[[1]]
        if (length(matches) > 3) {
            func_name <- matches[2]
            letter <- matches[3]
            number <- as.integer(matches[4]) + 1
            return(paste0("$", func_name, "(", letter, "_{", number, "})$"))
        }
    }

    # Handle function names like "sin(x1)" -> "$sin(x_2)$"
    if (grepl("^[a-zA-Z]+\\([a-zA-Z][0-9]+\\)$", term)) {
        matches <- regmatches(
            term,
            regexec("^([a-zA-Z]+)\\(([a-zA-Z])([0-9]+)\\)$", term)
        )[[1]]
        if (length(matches) > 3) {
            func_name <- matches[2]
            letter <- matches[3]
    ...
    # ~ ------------------------------------------------------------------------
    # Handle space-separated products with powers like "x0 x1 x2^2" -> "$x_1x_2x_3^2$"
    if (grepl("\\s+", term)) {
        # First, handle individual variables with powers in space-separated context
        term <- gsub("([a-zA-Z])([0-9]+)\\^([0-9]+)", "\\1_{\\2}^{\\3}", term)
        ...
        # Increment all subscripts by 1
        matches <- gregexpr("_\\{([0-9]+)\\}", term)
        if (matches[[1]][1] != -1) {
            for (i in length(matches[[1]]):1) {
                ...
                num <- as.integer(gsub("_\\{|\\}", "", matched_text))
                replacement <- paste0("_{", num + 1, "}")
                term <- paste0(
                    substr(term, 1, match_start - 1),
                    replacement,
                    substr(term, match_start + match_length, nchar(term))
                )
            }
        }
    ...
    # Handle function names with underscores like "sin_x1", "cos_x1", etc.
    if (grepl("^[a-zA-Z]+_[a-zA-Z][0-9]+$", term)) {
        matches <- regmatches(
            term,
            regexec("^([a-zA-Z]+)_([a-zA-Z])([0-9]+)$", term)
        )[[1]]
        if (length(matches) > 3) {
            func_name <- matches[2]
            letter <- matches[3]
    ...
    # Handle terms with powers like "x2^2", "x1^3", etc. -> "$x_3^2$", "$x_2^3$"
    if (grepl("^[a-zA-Z][0-9]+\\^[0-9]+$", term)) {
        matches <- regmatches(
            term,
            regexec("^([a-zA-Z])([0-9]+)\\^([0-9]+)$", term)
        )[[1]]
        if (length(matches) > 3) {
            letter <- matches[2]
            subscript <- as.integer(matches[3]) + 1
    ...
    # Handle simple variable terms like "x1" -> "$x_2$"
    if (grepl("^[a-zA-Z][0-9]+$", term)) {
        matches <- regmatches(term, regexec("^([a-zA-Z])([0-9]+)$", term))[[1]]
        if (length(matches) > 2) {
            letter <- matches[2]
            number <- as.integer(matches[3]) + 1
            return(paste0("$", letter, "_{", number, "}$"))
        }
    }

    # ~ ------------------------------------------------------------------------
    # Handle existing LaTeX terms - increment subscripts if present
    if (grepl("_\\{[0-9]+\\}", term)) {
        matches <- gregexpr("_\\{([0-9]+)\\}", term)
        if (matches[[1]][1] != -1) {
            for (i in length(matches[[1]]):1) {
    ...
    # Handle single digit subscripts like "x_1" -> "x_2"
    if (grepl("_[0-9]", term)) {
        matches <- gregexpr("_([0-9])", term)
        if (matches[[1]][1] != -1) {
            for (i in length(matches[[1]]):1) {
    ...
    # Final LaTeX wrapping if needed
    if ((grepl("_", term) || grepl("\\\\", term)) && !grepl("^\\$", term)) {
        term <- paste0("$", term, "$")
    }
...
convert_term_to_latex <- convert_term_to_latex_complicated

count_terms_identified_of_single_function_at_specific_eta_with_latex <- function(
    total_results_summary_df,
    method_name_list,
    selected_eta_list,
    function_id_list
...
plot_freq_of_identified_terms_with_highlighting_of_ground_truth_terms_of_all_equations_latex <- function(
    ...
    num_terms_rows <- length(seq(start, end, by = by))
    plots <- list()
    for (i in seq(1:function_number)) {
        for (j in seq(start, end, by = by)) {
            term_counts_df <- count_terms_identified_of_single_function_at_specific_eta_with_latex(
                total_results_summary_df = total_results_summary_df,
                method_name_list = list(method_name),
                function_id_list = list(i),
                selected_eta_list = list(j)
            )
            ...
            barchart_plot <- ggplot(
                ...
                scale_fill_gradient(low = "#fff799", high = "#d6bc46") +
                scale_color_manual(
                ...
                scale_y_discrete(labels = function(x) TeX(x)) +
                ggtitle(bquote(
                    "Number of identified terms of Equation" ~ .(i) * "on" ~
                        10^.(j) ~
                        "Observations"
                ))
...
plot_freq_of_identified_terms_with_highlighting_of_ground_truth_terms_of_single_equation_latex <- function(
    ...
    for (j in seq(start, end, by = by)) {
        ...
        barchart_plot <- ggplot(
            ...
            scale_y_discrete(labels = function(x) TeX(x)) +
            ggtitle(bquote(
                "Cumulated Number of Identified Terms" ~ "on" ~ 10^.(j) ~
                    "Observations"
            ))
...
plot_freq_of_identified_terms_circular_layered <- function(
    ...
    eta_values <- seq(start, end, by = by)
    num_eta_values <- length(eta_values)
    ...
    # Set up sectors - one for each eta value
    sectors <- paste0("Eta", eta_values)
    # sector_colors <- brewer.pal(min(num_eta_values, 11), "Pastel1")
    # if (num_eta_values > 11) {
    #     sector_colors <- rainbow(num_eta_values, alpha = 0.4)
    # }

    # Create custom gradually changing colors from start to end color
    sector_colors <- colorRampPalette(c("#aed0ee", "#06436f"))(num_eta_values)
    ...
    # Track 1: Eta value labels with colored background and number of observations
    circos.track(
        factors = sectors,
        ylim = c(0, 1),
        track.height = 0.05,
        ...
        panel.fun = function(x, y) {
            ...
            eta_val <- as.numeric(gsub("Eta", "", sector_name))

            # Calculate number of observations
            # n_obs <- 10^eta_val

            # Create proper expression for eta label
            eta_label <- paste0("10^", eta_val)
            eta_label <- parse(text = eta_label)
            # eta_label <- paste0(eta_label, "observations")
    ...
    for (eq_idx in seq(1:function_number)) {
        ...
        circos.track(
            ...
            panel.fun = function(x, y) {
                ...
                # Get data for this equation and eta combination
                eq_eta_data <- combined_data[
                    combined_data$function_id == eq_idx &
                        combined_data$eta == eta_val,
                ]

                if (nrow(eq_eta_data) > 0) {
                    # Sort by highlight status first, then frequency
                    eq_eta_data <- eq_eta_data[
                        order(
                            eq_eta_data$highlight == "ground_truth",
                            eq_eta_data$Freq,
                            decreasing = TRUE
                        ),
                    ]
                    ...
                    for (term_idx in seq_len(num_terms)) {
                        ...
                        if (term_data$Freq > track_y_max * 0.15) {
                            # Calculate adaptive cex based on number of terms
                            # More terms = smaller text, fewer terms = larger text
...
plot_freq_of_identified_terms_circular_equations_as_sectors <- function(
    ...
    use_sector_colors = TRUE # New parameter: use sector colors instead of eta colors for bars
) {
    true_terms_list <- list(
        true_terms_1,
        true_terms_2,
    ...
    # Generate eta values based on input method
    if (!is.null(eta_list)) {
        # Use custom eta list
        eta_values <- unlist(eta_list)
        cat(
            "Using custom eta values:",
    ...
    # Order eta values based on parameter
    if (eta_order == "descending") {
        eta_values <- rev(eta_values)
    }

    num_eta_values <- length(eta_values)

    # Calculate layer heights - either uniform or variable based on eta values
    if (variable_layer_height) {
        # Calculate weights inversely proportional to eta values
        # Smaller eta values get larger weights (and thus larger layer heights)
        eta_weights <- 1 / (eta_values^height_scaling_factor)

        # Normalize weights so they sum to 1
        eta_weights <- eta_weights / sum(eta_weights)
        ...
        layer_heights <- eta_weights * available_space
        ...
        cat("Variable layer heights (small eta = larger height):\n")
        ...
        layer_heights <- rep(dynamic_layer_height, num_eta_values)
    }

    # Collect all data
    all_data <- list()
    max_freq_global <- 0
    max_freq_by_eta <- rep(0, num_eta_values)
    ...
    for (i in seq(1:function_number)) {
        for (j in eta_values) {
            ...
            if (nrow(term_counts_df) > 0) {
                ...
                eta_idx <- which(eta_values == j)
                max_freq_by_eta[eta_idx] <- max(
                    max_freq_by_eta[eta_idx],
                    max(term_counts_df$Freq)
                )
            }
        }
    ...
    sector_colors <- colorRampPalette(c("#D3A237", "#D5B45C", "#ECD452"))(
        function_number
    )

    # Initialize with optimized spacing for up to 6 eta values
    gap_degree <- max(2, min(6, 15 / num_eta_values)) # Increased gaps for better separation
    ...
    # Color scheme for eta tracks - gradient from inner to outer
    if (eta_order == "ascending") {
        eta_colors <- colorRampPalette(c("#88abda", "#5976ba", "#145ca0"))(
            num_eta_values
        )
    } else {
        eta_colors <- colorRampPalette(c("#145ca0", "#5976ba", "#88abda"))(
            num_eta_values
        )
    }
    ...
    if (use_sector_colors) {
        ...
        for (eq_idx in 1:function_number) {
            # Create variations of the sector color for different eta values
            base_color <- sector_colors[eq_idx]
            # Generate shades from lighter to darker based on eta order
            if (eta_order == "ascending") {
        ...
        cat("Using eta-specific colors for bars across all sectors\n")
    }
    # Create tracks for each eta value (from inner to outer)
    for (eta_idx in seq_along(eta_values)) {
        eta_val <- eta_values[eta_idx]

        # Get the specific layer height for this eta value
        current_layer_height <- layer_heights[eta_idx]

        # Choose scaling: global or per-eta - reduced from 1.2 to 1.05 for shorter bars
        track_y_max <- if (normalize_by_layer) {
            max_freq_by_eta[eta_idx] * 1.5
        } else {
            max_freq_global * 1.5
        }
                    ...
                    panel.fun = function(x, y) {
                        ...
                        if (nrow(eq_eta_data) > 0) {
                            ...
                            if (num_terms == 1) {
                                ...
                                # Two terms - distribute evenly
                                x_positions <- c(0.25, 0.75)
                                bar_width <- 0.4
                            } else if (num_terms == 3) {
                                # Three terms - distribute evenly
                                x_positions <- c(0.15, 0.5, 0.85)
                                bar_width <- 0.3
                            } else {
                                # Multiple terms - distribute evenly
                            ...
                            for (term_idx in seq_len(num_terms)) {
                                ...
                                if (term_data$highlight == "ground_truth") {
                                    ...
                                    if (use_sector_colors) {
                                        ...
                                        fill_color <- eta_colors[eta_idx] # Eta-specific color
                                        border_color <- adjustcolor(
                                            eta_colors[eta_idx],
                                            alpha.f = 0.8
                                        )
                        ...
                        # Add eta label with improved sizing and positioning
                        if (CELL_META$sector.numeric.index == 1) {
                            eta_label <- paste0("10^", eta_val)
                            eta_label <- parse(text = eta_label)
                            circos.text(
    ...
    if (use_sector_colors) {
        ...
        # Original eta-based legend
        legend_labels <- c(
            "Ground Truth",
            paste0("η = 10^", eta_values)
        )
        legend_colors <- c("#9e9ac8", eta_colors) # Match ground truth color
        legend_borders <- c("#766bb1", adjustcolor(eta_colors, alpha.f = 0.8))
...
plot_freq_of_identified_terms_circular_equations_as_sectors_enhanced <- function(
    total_results_summary_df = total_results_summary_df,
    function_number = function_number,
    true_terms_1 = NULL,
    true_terms_2 = NULL,
    ...
    show_frequency = FALSE, # New parameter: use sector colors instead of eta colors for bars
    ...
    for (i in seq(1:function_number)) {
        for (j in eta_values) {
            ...
            if (nrow(term_counts_df) > 0) {
                ...
                # Find the eta index for this j value
                eta_idx <- which(eta_values == j)
                max_freq_by_eta[eta_idx] <- max(
                    max_freq_by_eta[eta_idx],
                    max(term_counts_df$Freq)
...
plot_freq_of_identified_terms_circular_equations_as_sectors_refined <- function(
    ...
    # Color scheme for eta tracks - gradient from inner to outer
    if (eta_order == "ascending") {
        eta_colors <- colorRampPalette(c("#5976ba"))(
            num_eta_values
        )
        eta_shrinkage_factor <- seq(
            from = 0.9,
            to = 0.6,
            length.out = num_eta_values
        )
    } else {
        eta_colors <- colorRampPalette(c("#5976ba"))(
            num_eta_values
    ...
    if (use_sector_colors) {
        ...
        for (eq_idx in 1:function_number) {
            ...
            if (eta_order == "ascending") {
                ...
                sector_color_palettes[[eq_idx]] <- sapply(
                    seq(0.7, 1, length.out = num_eta_values),
                    function(a) {
                ...
                # Inner (dark) to outer (light)
                sector_color_palettes[[eq_idx]] <- sapply(
                    seq(1, 0.7, length.out = num_eta_values),
                    function(a) {
                        adjustcolor(base_color, alpha.f = a)
                    }
                )
        ...
        for (eq_idx in 1:function_number) {
            ...
            if (eta_order == "ascending") {
                ...
                # )(num_eta_values)
                sector_color_palettes[[eq_idx]] <- sapply(
                    seq(0.5, 0.6, length.out = num_eta_values),
                    function(a) {
                        adjustcolor(base_color, alpha.f = a)
                    }
                )
                ...
                # Inner (dark) to outer (light)
                sector_color_palettes[[eq_idx]] <- sapply(
                    seq(0.6, 0.5, length.out = num_eta_values),
                    function(a) {
                        adjustcolor(base_color, alpha.f = a)
                    }
                )
                ...
                circos.track(
                    ...
                    panel.fun = function(x, y) {
                        ...
                        if (nrow(eq_eta_data) > 0) {
                            ...
                            # Simplified positioning: create linear alignment from inner to outer
                            # Calculate bar width based on maximum possible terms across all equations
                            if (eta_idx >= 5 && num_terms > 4) {
                                bar_width <- 2.2 / max_num_terms
                            } else if (eta_idx >= 4 && num_terms > 8) {
                                bar_width <- 1.2 / max_num_terms
                            } else {
                            ...
                            eta_shrinkage_factor <- eta_shrinkage_factor[
                                eta_idx
                            ] *
                                base_cex_multiplier
                            ...
                            adaptive_cex_terms <- if (num_terms <= 2) {
                                0.6 * eta_shrinkage_factor # Increased from 0.7
                            } else if (num_terms <= 4) {
                                0.55 * eta_shrinkage_factor # Increased from 0.6
                            } else if (num_terms <= 6) {
                                0.5 * eta_shrinkage_factor # Increased from 0.5
                            } else if (num_terms <= 8) {
                                0.5 * eta_shrinkage_factor # Increased from 0.4
                            } else {
                                0.45 * eta_shrinkage_factor # Increased from 0.35
                            }
                        ...
                        if (CELL_META$sector.numeric.index == 1) {
                            ...
                            if (eta_idx == 1) {
                                vary_factor <- paste0("Observations")
                                circos.text(
                                    x = 0.0,
                                    y = track_y_max + 80,
                                    labels = vary_factor,
...
# ~ ----------------------------------------------------------------------------
plot_freq_of_identified_terms_circular_equations_as_sectors_refined_2 <- function(
    total_results_summary_df = total_results_summary_df,
    function_number = function_number,
    true_terms_1 = NULL,
    true_terms_2 = NULL,
    ...
    min_freq_for_label = 0.05,
    base_cex_basis = 2.5,
    base_cex_adjustment = 0.75,
    adjustment_for_labels = 1.0,
    use_special_positioning = FALSE,
    normalize_by_layer = FALSE,
    layer_height = 0.25, # Increased from 0.3 for better visibility
    eta_order = "ascending",
    variable_layer_height = FALSE,
    ...
    circos.track(
        ...
        panel.fun = function(x, y) {
            ...
            circos.text(
                ...
                y = 0.5,
                labels = eq_label,
                cex = 1.5, # Increased from 0.8 for better readability
                col = "white",
                font = 2,
                facing = "inside",
                niceFacing = TRUE
                ...
                circos.track(
                    ...
                    panel.fun = function(x, y) {
                        ...
                        if (nrow(eq_eta_data) > 0) {
                            ...
                            # Simplified positioning: create linear alignment from inner to outer
                            # Calculate bar width based on maximum possible terms across all equations
                            if (use_special_positioning == FALSE) {
                                bar_width <- min(0.7 / num_terms, 0.12)
                            } else {
                                if (eta_idx >= 5 && num_terms > 4) {
                                    bar_width <- 0.7 / max_num_terms
                                } else if (eta_idx >= 4 && num_terms > 8) {
                                    bar_width <- 0.8 / max_num_terms
                                } else {
                                    bar_width <- min(0.8 / max_num_terms, 0.12)
                                }
                            }
                            ...
                            if (num_terms == 1) {
                                # Single term - position it towards the inner side
                                x_positions <- 0.1 + bar_width / 2
                                # --- Add typically for complex identification
                            } else {
                                if (use_special_positioning == TRUE) {
                                    start_pos <- 0.01 + bar_width / 2
                                    end_pos <- min(
                                        0.99 - bar_width / 2,
                                        start_pos +
                                            (num_terms - 1) * bar_width * 1.1
                            ...
                            base_cex_multiplier_orig <- base_cex_multiplier
                            base_cex_multiplier <- base_cex_multiplier *
                                adjustment_for_labels
                            eta_shrinkage_factor <- eta_shrinkage_factor[
                                eta_idx
                            ] *
                                base_cex_multiplier
                            ...
                            if (use_special_positioning == FALSE) {
                                adaptive_cex_bar <- if (num_terms <= 2) {
                                    1.0 * base_cex_multiplier_orig # Increased from 0.85
                                } else if (num_terms <= 4) {
                                    0.9 * base_cex_multiplier_orig # Increased from 0.75
                                } else if (num_terms <= 6) {
                                    0.6 * base_cex_multiplier_orig # Increased from 0.65
                                } else if (num_terms <= 8) {
                                    0.5 * base_cex_multiplier_orig # Increased from 0.55
                                } else if (num_terms <= 10) {
                                    0.5 * base_cex_multiplier_orig # Increased from 0.55
                                } else if (num_terms <= 15) {
                                    0.3 * base_cex_multiplier_orig # Increased from 0.55
                                } else {
                                    0.15 * base_cex_multiplier_orig # Increased from 0.45
                                }
                            } else {
                                adaptive_cex_bar <- if (num_terms <= 2) {
                                    1.0 * base_cex_multiplier_orig # Increased from 0.85
                            ...
                            if (use_special_positioning == FALSE) {
                                adaptive_cex_terms <- if (num_terms <= 2) {
                                    0.65 * eta_shrinkage_factor # Increased from 0.7
                                } else if (num_terms <= 4) {
                                    0.55 * eta_shrinkage_factor # Increased from 0.6
                                } else if (num_terms <= 6) {
                                    0.55 * eta_shrinkage_factor # Increased from 0.5
                                } else if (num_terms <= 8) {
                                    0.55 * eta_shrinkage_factor # Increased from 0.4
                                } else {
                                    0.55 * eta_shrinkage_factor # Increased from 0.35
                                }
                            } else {
                                adaptive_cex_terms <- if (num_terms <= 2) {
                                    0.6 * eta_shrinkage_factor # Increased from 0.7
                                } else if (num_terms <= 4) {
                                    0.5 * eta_shrinkage_factor # Increased from 0.6
                                } else if (num_terms <= 6) {
                                    0.5 * eta_shrinkage_factor # Increased from 0.5
                                } else if (num_terms <= 8) {
                                    0.45 * eta_shrinkage_factor # Increased from 0.4
                                } else {
                                    0.35 * eta_shrinkage_factor # Increased from 0.35
                                }
                            }
                        ...
                        if (CELL_META$sector.numeric.index == 1) {
                            ...
                            if (eta_idx == 1) {
                                ...
                                circos.text(
                                    ...
                                    y = track_y_max + 80,
                                    labels = vary_factor,
                                    cex = 1.075 * base_cex_multiplier_orig, # Increased from 0.6
                                    col = eta_colors[eta_idx],
                                    font = 2,
                                    adj = c(0.55, 0),
                                    facing = "inside",
                            ...
                            circos.text(
                                ...
                                y = track_y_max / 2,
                                labels = eta_label,
                                cex = 0.7 * base_cex_multiplier_orig, # Increased from 0.6
                                col = eta_colors[eta_idx],
                                font = 2,
                                adj = c(0.5, 0),
                                facing = "inside",
