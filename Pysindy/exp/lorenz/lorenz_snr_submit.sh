...
for loop_var in $snr_loop_seq; do
    sbatch --mem=${memory}G --export=ALL,N_OBS=${n_obs},NUM_INIT=${num_init},START=${loop_var},END=$(echo "$loop_var+$shell_by" | bc),BY_SNR=${snr_by},DT=${dt},SEED=${seed},POLY_ORDER=${sg_poly_order},LIBRARY_DEGREE=${library_degree},LIBRARY_TYPE=${library_type},CPU_NUM=${ncpus} lorenz_snr.sh
done