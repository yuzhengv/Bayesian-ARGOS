...
system <- "dadras-test"
system_coeff <- list(list(1, -3, 2.7), list(1.7, -1, 1), list(2, -9))
system_names <- list(list("x2", "x1", "x2x3"), list("x2", "x1x3", "x3"), list("x1x2", "x3"))
...
get_exp_results <- function(
    ...
    model <- ps$SINDy(feature_library = polynomial_library, optimizer=ensemble_optimizer)

    model_fit <- model$fit(x = smoothed_data, t = dt, x_dot = smoothed_data_dot, ensemble=TRUE, quiet=TRUE)
