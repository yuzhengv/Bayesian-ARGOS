...
system <- "forced-duffing"
system_coeff <- list(list(1), list(-1, -0.1, -5, 5))
system_names <- list(list("x2"), list("x1", "x2", "x1^3", "cos(2t)"))
...
get_exp_results <- function(
    ...
    # - External forcing input for SINDYc (control library includes u0)
    t_vec <- seq(0, by = dt, length.out = nrow(smoothed_data))
    u_forcing <- matrix(cos(2 * t_vec), ncol = 1)
    ...
    model_fit <- model$fit(x = smoothed_data, t = dt, x_dot = smoothed_data_dot, u = u_forcing)
