...
extract_info <- function(method, concatenated_lists) {
    ...
    if (
        ...
        identified_model_list_classified <- lapply(
            ...
            function(x) {
                lapply(
                    x,
                    function(y) y$id_result$argos_bi_output$identified_model
                )
            }
        )
    ...
    } else if (method %in% c("argos-lasso", "argos-alasso")) {
        identified_model_list_classified <- lapply(
            concatenated_lists$concatenated_list,
            function(x) {
                lapply(x, function(y) y$id_result$argos_output$identified_model)
            }
        )
    ...
    return(
        list(
            # identified_model_list_classified = identified_model_list_classified,
            identified_model_list = identified_model_list_unlisted,
            # compute_time_list_classified = compute_time_list_classified,
...
extract_info_df <- function(concatenated_lists, function_id, method) {
    if (method %in% c("argos-lasso", "argos-alasso")) {
        method <- concatenated_lists$metadata[[1]]$sr_method
...
results_loader_and_combinator <- function(
    ...
    for (i in seq(experiments_results_all_list)) {
        concatenated_all_lists[[i]] <- concatenate_lists_parl(
            experiments_results_all_list[[i]]
        )
    }
...
# ~ ----------------------------------------------------------------------------
# ~ Time complexity table generation
...
create_time_complexity_table <- function(
...
# ~ ----------------------------------------------------------------------------
# ~ This function is used to check whether there are errors in
# ~ the original running result for a specific function
# ~ Not for main results processing
view_original_running_result_for_specific_function <- function(
    root_path,
    experiment_name,
    method,
    dynamical_system_name,
    ...
    extracted_info <- extract_info(method, concatenated_lists)
    identified_model_list <- extracted_info$identified_model_list
    return(
        list(
            concatenated_lists = concatenated_lists,
            extracted_info = extracted_info,
            identified_model_list = identified_model_list
        )
    )
}

# ~ ----------------------------------------------------------------------------
# ~ The main success rate table generation functions
...
model_correctness_checker <- function(identified_model_list, true_terms) {
    ...
    trimmed_model_list <- sapply(seq_along(identified_model_list), function(i) {
        ...
        as.matrix(identified_model_list[[i]][identified_model_list[[i]] != 0, ])
    })
    # Identify whether the true terms are in the identified model
    ...
    for (i in seq_along(trimmed_model_list)) {
        ...
        match_index <- match(rownames(trimmed_model_list[[i]]), true_terms)
        if (
    ...
    return(identified_bool_list)
}

dynamical_system_checker <- function(
    root_path,
    experiment_name,
    method,
    function_number,
    ...
    identified_indicator_df <- as.data.frame(identified_indicator_matrix) # data.frame
...
# ~ Augmented success rate table generation function for quick and concise results
# ~ data processing
# - This function is used to generate a success rate table for a specific sparse regression method
generate_separate_success_rate_table <- function(
    root_path = root_path,
    experiment_name = experiment_name,
    method = method,
    function_number = function_number,
    ...
    dynamics_identification_results <- dynamical_system_checker(
        root_path = root_path,
        experiment_name = experiment_name,
        method = method,
        function_number = function_number,
    ...
    success_rate_table <- build_successful_rate_table(
...
# - This function is used to generate a success rate table for a list of sparse regression methods
generate_total_success_rate_table <- function(
    root_path = root_path,
    experiment_name_list = experiment_name_list,
    method_list = method_list,
    function_number = function_number,
    ...
    experiment_method_match <- data.frame(
        experiment_name = unlist(experiment_name_list),
        method = unlist(method_list)
    )
    total_success_rate_table <- do.call(
        rbind,
        lapply(seq_len(nrow(experiment_method_match)), function(i) {
            generate_separate_success_rate_table(
                root_path = root_path,
                experiment_name = experiment_method_match$experiment_name[i],
                method = experiment_method_match$method[i],
                function_number = function_number,
                num_init = num_init,
                dynamical_system_name = dynamical_system_name,
                exp_n = exp_n,
    ...
    return(total_success_rate_table)
}
