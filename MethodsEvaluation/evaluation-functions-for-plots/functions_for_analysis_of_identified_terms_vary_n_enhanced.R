...
convert_term_to_latex <- function(term) {
    ...
    # Return early if already in LaTeX format
    if (grepl("^\\$.*\\$$", term)) {
        return(term)
    }
    ...
    # Handle constants and simple terms without numbers
    if (
        grepl("^[a-zA-Z]+$", term) && !grepl("^(sin|cos|tan|log|exp|ln)$", term)
    ) {
        # Keep simple variable names as is, or add LaTeX if needed
        if (nchar(term) == 1) {
            term <- paste0("$", term, "$")
        }
    }

    # If we have underscores or superscripts, wrap in LaTeX
...
nbfigsize <- function(width, height, dpi = 300) {
    options(
...
plot_freq_of_identified_terms_of_all_equations <- function(
    ...
    for (i in seq(1:function_number)) {
        for (j in seq(start, end, by = by)) {
            term_counts_df <- count_terms_identified_of_single_function_at_specific_eta(
                total_results_summary_df = total_results_summary_df,
    ...
    for (i in seq(1:function_number)) {
        for (j in seq(start, end, by = by)) {
            ...
            barchart_plot <- ggplot(
                ...
                ggtitle(bquote(
                    "Governing Equation" ~ .(i) * "," ~ 10^.(j) ~ "Observations"
                ))
    ...
    for (i in seq(1:function_number)) {
        for (j in seq(start, end, by = by)) {
            ...
            barchart_plot <- ggplot(
                ...
                scale_y_discrete(labels = function(x) TeX(x)) +
                ggtitle(bquote(
                    "Identified Terms of Equation" ~ .(i) * ~"on" ~ 10^.(j) ~
                        "Observations"
                ))
...
plot_freq_of_identified_terms_with_highlighting_of_ground_truth_terms_of_single_equation_latex <- function(
    ...
    for (j in seq(start, end, by = by)) {
        ...
        barchart_plot <- ggplot(
            ...
            scale_y_discrete(labels = function(x) TeX(x)) +
            ggtitle(bquote(
                "Identified Terms of Equation" ~
                    .(specific_function_id) * ~"on" ~
                    10^.(j) ~
                    "Observations"
            ))
...
# # Save the plot as a grob object
# my_plot_grob <- plot_freq_of_identified_terms_with_highlighting_of_ground_truth_terms_of_single_equation_latex(
#     # ... your parameters
# )

# # Display it when needed
# grid.draw(my_plot_grob)

# # Or combine it with other plots
# final_plot <- grid.arrange(my_plot_grob, other_plot, ncol = 2)

# # Or use it in more complex layouts
# grid.arrange(
#     my_plot_grob,
...
plot_freq_of_identified_terms_circular_layered <- function(
    ...
    all_data <- list()
    max_freq_global <- 0
    max_freq_by_eta <- rep(0, function_number)
            ...
            panel.fun = function(x, y) {
                ...
                if (nrow(eq_eta_data) > 0) {
                    ...
                    if (num_terms == 1) {
                        ...
                        end_pos <- min(
                            0.9 - bar_width / 2,
                            start_pos + (num_terms - 1) * bar_width * 1.1
                        )
                        x_positions <- seq(
                            start_pos,
                            end_pos,
...
# ------------------------------------------------------------------------------
# plot_freq_of_identified_terms_circular_equations_as_sectors_refined_specific <- function(
#     total_results_summary_df = total_results_summary_df,
#     function_number = function_number,
#     true_terms_1 = NULL,
#     true_terms_2 = NULL,
...
#         start.degree = 90,
#         gap.degree = gap_degree,
#         track.margin = c(0, 0), # Minimal margins
#         cell.padding = c(0, 0, 0, 0) # No inner padding
#     )
