...
metadata <- list(
    ...
    ci_level = ci_level,
    ablation_variant = "bayesian-alasso-or"
)
...
process_bayesian_argos_thomas <- function(n,
    ...
    start_time <- Sys.time()
    perform_bayesian_argos <- bayesian_alasso_or(
...
save(
    ...
    metadata,
    file = sprintf(
        "./Experiments/bayesian-alasso-or/thomas/results/thomas_%s_snr%s_snr%s_se%s_n%s.RData",
        state_var_deriv,
        snr_start,
        snr_end,
        seed,
