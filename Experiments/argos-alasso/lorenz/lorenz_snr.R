...
process_argos_lorenz <- function(n,
                                 init_conditions,
                                 dt,
                                 snr,
                                 sg_poly_order,
...
for (i in seq_along(snr_value)) {
    ...
    snr_output[[i]] <- mclapply(seq_along(init_sequence), function(j) {
        run_id <- c(i = i, j = j)
        print(run_id)
        id_result <- process_argos_lorenz(
            n = n_obs,
...
if (sr_method == "lasso") {
    save(
        ...
        metadata,
        file = sprintf(
            "./Experiments/argos-alasso/lorenz/results/lorenz_%s_snr%s_snr%s_bo%s_se%s_%s_n%s.RData",
            state_var_deriv,
            snr_start,
            snr_end,
            num_samples,
    ...
    save(
        ...
        metadata,
        file = sprintf(
            "./Experiments/argos-alasso/lorenz/results/lorenz_%s_snr%s_snr%s_bo%s_se%s_%s_%s_n%s.RData",
            state_var_deriv,
            snr_start,
            snr_end,
            num_samples,
