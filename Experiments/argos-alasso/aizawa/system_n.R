...
for (i in seq_along(n_obs)) {
    ...
    n_output[[i]] <- mclapply(
        ...
        function(j) {
            run_id <- c(i = i, j = j)
            print(run_id)
            id_result <- process_argos_system(
                n = n_obs[i],
...
if (sr_method == "lasso") {
    save(
        ...
        metadata,
        file = sprintf(
            "./Experiments/argos-lasso/aizawa/results/aizawa_%s_e%s_e%s_bo%s_se%s_%s_snr%s_bigmem.RData",
            state_var_deriv,
            start,
            end,
            num_samples,
    ...
    save(
        ...
        metadata,
        file = sprintf(
            "./Experiments/argos-alasso/aizawa/results/aizawa_%s_e%s_e%s_bo%s_se%s_%s_%s_snr%s_bigmem.RData",
            state_var_deriv,
            start,
            end,
            num_samples,
