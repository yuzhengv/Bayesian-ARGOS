...
process_argos_duffing <- function(n,
                                  init_conditions,
                                  dt,
                                  snr,
                                  sg_poly_order,
                                  ...
                                  parallel,
                                  ncpus) {
    # Duffing oscillator: dx1 = x2; dx2 = -kappa*x1 - gamma*x2 - epsilon*x1^3
    # with gamma = 0.1, kappa = 1, epsilon = 5
    duffing_coeff <- list(list(1), list(-1, -0.1, -5))
    duffing_names <- list(list("x2"), list("x1", "x2", "x1^3"))
    xn <- generate_noisy_dynamical_systems_pyversion(
        variable_coeff = duffing_coeff,
        variable_names = duffing_names,
        n = n,
        dt = dt,
...
for (i in seq_along(snr_value)) {
    ...
    snr_output[[i]] <- mclapply(seq_along(init_sequence), function(j) {
        run_id <- c(i = i, j = j)
        print(run_id)
        id_result <- process_argos_duffing(
            n = n_obs,
...
if (sr_method == "lasso") {
    save(
        ...
        metadata,
        file = sprintf(
            "./Experiments/argos-lasso/duffing/results/duffing_%s_snr%s_snr%s_bo%s_se%s_%s_n%s.RData",
            state_var_deriv,
            snr_start,
            snr_end,
            num_samples,
    ...
    save(
        ...
        metadata,
        file = sprintf(
            "./Experiments/argos-alasso/duffing/results/duffing_%s_snr%s_snr%s_bo%s_se%s_%s_%s_n%s.RData",
            state_var_deriv,
            snr_start,
            snr_end,
            num_samples,
