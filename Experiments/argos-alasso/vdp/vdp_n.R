...
process_argos_vdp <- function(n,
                              ...
                              parallel,
                              ncpus) {
    vdp_coeff <- list(list(1), list(1.2, -1.2, -1))
    vdp_names <- list(list("x2"), list("x2", "x1^2x2", "x1"))
    xn <- generate_noisy_dynamical_systems_pyversion(
        variable_coeff = vdp_coeff,
        variable_names = vdp_names,
        n = n,
        dt = dt,
...
# todo pay attention to the specified init_conditions, should be specified by the inner loop running index
...
if (sr_method == "lasso") {
    save(
        ...
        metadata,
        file = sprintf(
            "./Experiments/argos-alasso/vdp/results/vdp_%s_e%s_e%s_bo%s_se%s_%s_snr%s.RData",
            state_var_deriv,
            start,
            end,
            num_samples,
    ...
    save(
        ...
        metadata,
        file = sprintf(
            "./Experiments/argos-alasso/vdp/results/vdp_%s_e%s_e%s_bo%s_se%s_%s_%s_snr%s.RData",
            state_var_deriv,
            start,
            end,
            num_samples,
