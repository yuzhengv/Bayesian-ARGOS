...
process_argos_sprott <- function(n,
                                 init_conditions,
                                 dt,
                                 snr,
                                 sg_poly_order,
                                 ...
                                 parallel,
                                 ncpus) {
    sprott_coeff <- list(list(1, 2.07, 1), list(1, -1.79, 1), list(1, -1, -1))
    sprott_names <- list(list("x2", "x1x2", "x1x3"), list("", "x1^2", "x2x3"), list("x1", "x1^2", "x2^2"))
    xn <- generate_noisy_dynamical_systems_pyversion(
        variable_coeff = sprott_coeff,
        variable_names = sprott_names,
        n = n,
        dt = dt,
...
for (i in seq_along(snr_value)) {
    ...
    snr_output[[i]] <- mclapply(seq_along(init_sequence), function(j) {
        run_id <- c(i = i, j = j)
        print(run_id)
        id_result <- process_argos_sprott(
            n = n_obs,
...
if (sr_method == "lasso") {
    save(
        ...
        metadata,
        file = sprintf(
            "./Experiments/argos-lasso/sprott/results/sprott_%s_snr%s_snr%s_bo%s_se%s_%s_n%s.RData",
            state_var_deriv,
            snr_start,
            snr_end,
            num_samples,
    ...
    save(
        ...
        metadata,
        file = sprintf(
            "./Experiments/argos-alasso/sprott/results/sprott_%s_snr%s_snr%s_bo%s_se%s_%s_%s_n%s.RData",
            state_var_deriv,
            snr_start,
            snr_end,
            num_samples,
