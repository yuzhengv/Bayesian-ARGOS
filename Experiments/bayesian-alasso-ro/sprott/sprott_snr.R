...
process_bayesian_argos_sprott <- function(n,
                                          init_conditions,
                                          dt, snr,
                                          sg_poly_order,
                                          library_degree,
...
for (i in seq_along(snr_value)) {
    ...
    snr_output[[i]] <- mclapply(seq_along(init_sequence), function(j) {
        run_id <- c(i = i, j = j)
        print(run_id)
        id_result <- process_bayesian_argos_sprott(
            n = n_obs,
...
save(
    ...
    metadata,
    file = sprintf(
        "./Experiments/bayesian-alasso-ro/sprott/results/sprott_%s_snr%s_snr%s_se%s_n%s.RData",
        state_var_deriv,
        snr_start,
        snr_end,
        seed,
