...
# the duffing system respectively with differnt number of observations.
for state_var in $state_var_seq; do
    for loop_var in $shell_loop_seq; do
        sbatch --mem=${memory}G --export=ALL,SNR=${snr},NUM_INIT=${num_init},START=${loop_var},END=$(echo "$loop_var+$shell_by" | bc),BY_TIME=${by_time},DT=${dt},SEED=${seed},POLY_ORDER=${sg_poly_order},LIBRARY_DEGREE=${library_degree},LIBRARY_TYPE=${library_type},STATE_VAR=${state_var},CI_LEVEL=${ci_level},CPU_NUM=${ncpus} forced_duffing_n.sh
    done
done
