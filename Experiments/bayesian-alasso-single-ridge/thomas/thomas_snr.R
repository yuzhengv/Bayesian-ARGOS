...
metadata <- list(
    ...
    ci_level = ci_level,
    ablation_variant = "bayesian-alasso-single-ridge"
)

################################################################################
##################### Define the testing-process function ######################
process_bayesian_argos_thomas <- function(n,
                                          init_conditions,
                                          dt, snr,
                                          sg_poly_order,
                                          library_degree,
    ...
    start_time <- Sys.time()
    perform_bayesian_argos <- bayesian_alasso_single(
...
for (i in seq_along(snr_value)) {
    ...
    snr_output[[i]] <- mclapply(seq_along(init_sequence), function(j) {
        run_id <- c(i = i, j = j)
        print(run_id)
        id_result <- process_bayesian_argos_thomas(
            n = n_obs,
...
save(
    ...
    metadata,
    file = sprintf(
        "./Experiments/bayesian-alasso-single-ridge/thomas/results/thomas_%s_snr%s_snr%s_se%s_n%s.RData",
        state_var_deriv,
        snr_start,
        snr_end,
        seed,
