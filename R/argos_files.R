  ...
  tryCatch(
    {
      ...
      if (
        lasso_init$lambda.min == lasso_init$lambda[length(lasso_init$lambda)]
      ) {
...
#' @importFrom glmnet cv.glmnet
alasso <-
  function(data, index, weights_method = c("ols", "ridge"), ols_ps = TRUE) {
    tryCatch(
      {
        x <- as.matrix(data[index, -1])
        y <- as.matrix(data[index, 1, drop = FALSE])
        ...
        if (weights_method == "ridge") {
          ...
          ridge_coef <- as.numeric(coef(ridge_init, s = ridge_init$lambda.min))
          if (
            ridge_init$lambda.min ==
            ...
            lambda_grid <-
              seq(upper_bound_grid, lower_bound_grid, length = 100)
            ridge_new <-
              cv.glmnet(x, y, alpha = 0, lambda = lambda_grid, intercept = TRUE)
...
sg_optimal_combination <- function(x_t, dt = 1, polyorder) {
  ...
  sg_order_wl <- cbind(
    sg_combinations[sg_best_combination, 1],
...
#'   build_design_matrix(x_t,
#'     dt = 0.01, sg_poly_order = 4,
...
argos <- function(
  ...
  new_theta_order <- sum(monomial_orders <= monomial_orders[init_nz_max])
  if (library_type == "four") {
    post_lasso_matrix <- data
  } else {
    # Rerun Bootstrap with Truncated Matrix
    if (
      is.na(new_theta_order) |
  ...
  boot_t <-
    matrix(as.numeric(boot_t), nrow = num_samples, ncol = ncol(boot_t))
  # subset any NAs
...
cubic2d_system <- function(n, init_conditions, dt, snr = Inf) {
  ...
  cubic2d <- function(t, init_conditions, parameters) {
    with(as.list(c(init_conditions, parameters)), {
      dx <- matrix_a[1, 1] *
        init_conditions[1]**3 +
        matrix_a[1, 2] * init_conditions[2]**3
      dy <- matrix_a[2, 1] *
        init_conditions[1]**3 +
        matrix_a[2, 2] * init_conditions[2]**3
      list(c(dx, dy))
    })
  }
  out <- ode(
  ...
  if (!is.infinite(snr)) {
    ...
    snr_volt <- 10^-(snr / 20)
    noise_matrix <- snr_volt *
      matrix(rnorm(length, mean = 0, sd = sd(out)), nrow(out))
...
linear2d_system <- function(n, init_conditions, dt, snr = Inf) {
  ...
  linear2d <- function(t, init_conditions, parameters) {
    with(as.list(c(init_conditions, parameters)), {
      dx <- matrix_a[1, 1] *
        init_conditions[1] +
        matrix_a[1, 2] * init_conditions[2]
      dy <- matrix_a[2, 1] *
        init_conditions[1] +
        matrix_a[2, 2] * init_conditions[2]
      list(c(dx, dy))
    })
  }
  out <- ode(
...
duffing_oscillator <-
  function(
    ...
    duff_osc <- function(t, init_conditions, duff_parameters) {
      with(
        as.list(c(
          init_conditions,
          duff_parameters
        )),
        {
          ...
          dy <-
            (-duff_parameters[1] * init_conditions[2]) -
            (duff_parameters[2] * init_conditions[1]) -
            (duff_parameters[3] * (init_conditions[1]^3))
          list(c(dx, dy))
        }
      )
    }
...
lotka_volterra <- function(n, init_conditions, dt, snr = Inf) {
  ...
  lv <- function(t, init_conditions, parameters) {
    with(as.list(c(init_conditions, parameters)), {
      dx <- (parameters[1] * init_conditions[1]) +
        (parameters[2] * (init_conditions[1] * init_conditions[2]))
      dy <- (parameters[3] * init_conditions[2]) +
        (parameters[4] * (init_conditions[1] * init_conditions[2]))
      list(c(dx, dy))
    })
  }
  times <- seq(0, ((n) - 1) * dt, by = dt)
...
lorenz_system <- function(n, init_conditions, dt, snr = Inf) {
  ...
  lorenz <- function(t, init_conditions, parameters) {
    with(as.list(c(init_conditions, parameters)), {
      ...
      dy <- init_conditions[1] *
        (parameters[2] - init_conditions[3]) -
        init_conditions[2]
      dz <- init_conditions[1] *
        init_conditions[2] -
        parameters[3] * init_conditions[3]
      list(c(dx, dy, dz))
    })
  }
  times <- seq(0, ((n) - 1) * dt, by = dt)
...
#' @export
rossler_system <-
  function(n, dt, init_conditions, a, b, c, snr = Inf) {
    n <- round(n, 0)
    dt <- dt
    # n = number of time points rounded to nearest integer
    # snr = added noise to system (dB)
    ...
    rossler <- function(t, init_conditions, rossler_parameters) {
      with(
        as.list(c(
          init_conditions,
          rossler_parameters
        )),
        {
          ...
          dz <-
            rossler_parameters[2] +
            (init_conditions[3] * (init_conditions[1] - rossler_parameters[3]))
          list(c(dx, dy, dz))
        }
      )
    }
