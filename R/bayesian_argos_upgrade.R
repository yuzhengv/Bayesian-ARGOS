double_regression <- function(data,
                              monomial_orders,
                              super_weights_method,
                              super_ols_ps = TRUE,
                              library_type = "poly") {
    target <- as.matrix(data[, 1, drop = FALSE])
    colnames(target) <- "target"
    initial_estimate_spr <- alasso(data, weights_method = super_weights_method, ols_ps = super_ols_ps)
    ...
    final_estimate_spr <- alasso(post_spr_matrix, weights_method = super_weights_method, ols_ps = super_ols_ps)
...
bayesian_argos <- function(design_matrix,
    ...
    # - Perform initial sparse regression to shrinkage the design matrix and make comparision between the results obtained from OLS and Ridge
    # ~ Add tryCatch mechanism to avoid the senario in which one of the sparse regression methods failed

    glm_ols_results <- tryCatch(
        glm_ols_results <- double_regression(data, monomial_orders = monomial_orders, super_weights_method = "ols", super_ols_ps = TRUE, library_type = library_type),
        error = function(e) NULL
    )
    glm_ridge_results <- tryCatch(
        glm_ols_results <- double_regression(data, monomial_orders = monomial_orders, super_weights_method = "ridge", super_ols_ps = TRUE, library_type = library_type),
        error = function(e) NULL
    )

    # ~ Shrinkage the design matrix by making comparisions between the results obtained from OLS and Ridge
    if (!is.null(glm_ols_results) & !is.null(glm_ridge_results)) {
    ...
    } else if (!is.null(glm_ols_results) & is.null(glm_ridge_results)) {
        coef_nonzero <- glm_ols_results$coef_nonzero
        final_spr_matrix <- glm_ols_results$final_spr_matrix
    } else if (is.null(glm_ols_results) & !is.null(glm_ridge_results)) {
        coef_nonzero <- glm_ridge_results$coef_nonzero
        ...
        return(list(
            identified_model = matrix(data = NA, nrow = 1),
            ci = matrix(data = NA, nrow = 1),
            coef = matrix(data = NA, nrow = 1),
            trimmed_model = matrix(data = NA, nrow = 1)
        ))
    }
