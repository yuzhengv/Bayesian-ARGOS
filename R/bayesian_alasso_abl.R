double_regression <- function(
    ...
    initial_estimate_spr <- alasso(
        data,
        weights_method = weights_method_1,
        ols_ps = super_ols_ps
    )
    initial_estimate_spr[is.na(initial_estimate_spr)] <- 0
    init_nz_max_spr <- max(which(initial_estimate_spr != 0))
    new_theta_order_spr <- sum(
        monomial_orders <= monomial_orders[init_nz_max_spr]
    )
    if (library_type == "four") {
        post_spr_matrix <- data
    } else {
        if (
            is.na(new_theta_order_spr) |
    ...
    final_estimate_spr <- alasso(
        post_spr_matrix,
        weights_method = weights_method_2,
        ols_ps = super_ols_ps
    )
    ...
    if (
        ...
        glm_result <-
            lm(
                target ~ as.matrix(selected_x),
                data = post_spr_matrix,
                na.action = na.fail
            )
...
single_regression <- function(data, weights_method, ols_ps = TRUE) {
    target <- as.matrix(data[, 1, drop = FALSE])
    colnames(target) <- "target"
    final_estimate_spr <- alasso(
        data,
    ...
    if (
        ...
        selected_x <- data[, -1][, final_nz_spr[-1], drop = FALSE]
        glm_result <-
            lm(
                target ~ as.matrix(selected_x),
                data = data,
        ...
        glm_result <-
            lm(target ~ 1, data = data, na.action = na.fail)
    } else if ((!final_nz_spr[1]) & any(final_nz_spr[-1])) {
        selected_x <- data[, -1][, final_nz_spr[-1], drop = FALSE]
        glm_result <-
            lm(
    ...
    return(list(
        glm_result = glm_result,
        final_spr_matrix = data,
        coef_nonzero = final_nz_spr
    ))
}

bayesian_alasso_or <- function(
    design_matrix,
    ...
    glm_ro_results <- double_regression(
        data,
        monomial_orders = monomial_orders,
        weights_method_1 = "ols",
        weights_method_2 = "ridge",
        super_ols_ps = TRUE,
...
bayesian_alasso_single <- function(
    design_matrix,
    target_distribution = gaussian(),
    ...
    # library_type is unused: the single-stage ablation performs no
    # library truncation. Kept so calls written for bayesian_alasso_*
    # run unchanged.
    ...
    #- Create the data set used for sparse regression
    deriv_col <- xdot[, state_var_deriv]
    data <- cbind.data.frame(deriv_col, sorted_theta)

    #- Ablation: single-stage sparse regression, no initial shrinkage
    #- of the design matrix
    glm_single_results <- single_regression(
        data,
        weights_method = weights_method,
        ols_ps = TRUE
    )

    coef_nonzero <- glm_single_results$coef_nonzero
    final_spr_matrix <- glm_single_results$final_spr_matrix
    ...
    #- Processing the posterior interval obtained from stan_glm
    post_interval <- posterior_interval(glm_stan, prob = ci_level)
    rownames(post_interval) <-
        sapply(rownames(post_interval), function(x) gsub("`", "", x))
    ci <- t(post_interval)

    #- Processing the coefficients obtained from stan_glm
    coef <- coef(glm_stan)
    names(coef) <- sapply(names(coef), function(x) gsub("`", "", x))
    coef <- as.matrix(coef)
    ...
    for (i in seq_along(coef)) {
        crosses_zero <-
            (ci[1, i] <= 0 && ci[2, i] >= 0) ||
            (ci[1, i] >= 0 && ci[2, i] <= 0)
        if (ci[1, i] <= coef[i] && ci[2, i] >= coef[i] && !crosses_zero) {
            identified_model[i, ] <- coef[i]
        } else {
            identified_model[i, ] <- 0
        }
    ...
    # remove zero rows
    trimmed_model <- as.matrix(identified_model[identified_model != 0, ])
    return(list(
        identified_model = identified_model,
        ci = ci,
        coef = coef,
